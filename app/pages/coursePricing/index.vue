<!-- app/pages/coursePricing/index.vue -->
<script setup lang="ts">

definePageMeta({
  title: 'Courses Pricing Management'
})
...
const itemToDelete = ref<CoursePricing | null>(null)
...
const handleView = (id: string | number) => {
  router.push(`/coursePricing/${id}`)
}

// Handle edit
const handleEdit = (id: string | number) => {
  router.push(`/coursePricing/${id}/edit`)
}

// Handle delete
const handleDelete = (coursePricing: CoursePricing) => {
  itemToDelete.value = coursePricing
...
const confirmDelete = async () => {
  ...
  try {
    await crud.deleteItem(itemToDelete.value.pricing_id)
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Manage your course Pricing and their permissions
        </p>
      </div>
      <UButton icon="i-heroicons-plus" size="lg" class="w-full sm:w-auto" @click="router.push('/coursePricing/create')">
        Create Course pricing
      </UButton>
    </div>

    <!-- Search -->
    <UCard v-if="CoursePricingResource.searchable">
      <UInput v-model="searchQuery" icon="i-heroicons-magnifying-glass" placeholder="Search Courses Prices..."
        size="lg" />
    </UCard>
    ...
    <!-- Table -->
    <CrudsCrudTable :config="CoursePricingResource" :items="[...crud.items.value]" :loading="crud.loading.value"
        ...
        <div class="space-y-3">
          ...
          <div v-if="itemToDelete" class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">
              {{ itemToDelete.pricing_id || 'Course Price' }}
            </p>
