<!-- app/pages/userProgress/index.vue -->
...
const itemToDelete = ref<UserProgress | null>(null)
...
const handleView = (id: string | number) => {
  router.push(`/userProgress/${id}`)
}

// Handle edit
const handleEdit = (id: string | number) => {
  router.push(`/userProgress/${id}/edit`)
}

// Handle delete
const handleDelete = (userProgress: UserProgress) => {
  itemToDelete.value = userProgress
...
const confirmDelete = async () => {
  ...
  try {
    await crud.deleteItem(itemToDelete.value.progress_id)
    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Manage your User Progress and their permissions
        </p>
      </div>
    </div>

    <!-- Search -->
    <UCard v-if="userProgressResource.searchable">
      <UInput
        v-model="searchQuery"
        ...
        <div class="space-y-3">
          ...
          <div v-if="itemToDelete" class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">
              {{ itemToDelete.lesson_id || 'Course Price' }}
            </p>
          </div>
        </div>
      </template>
