<!-- app/pages/sections/index.vue -->
<script setup lang="ts">
import { sectionsResource, type Section } from '~/config/resources/sections'
...
const confirmDelete = async () => {
  ...
  try {
    await crud.deleteItem(itemToDelete.value.section_id)
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Manage your Sections and their permissions
        </p>
      </div>
      <UButton icon="i-heroicons-plus" size="lg" class="w-full sm:w-auto" @click="router.push('/sections/create')">
        Create section
      </UButton>
    </div>

    <!-- Search -->
    <UCard v-if="sectionsResource.searchable">
      <UInput v-model="searchQuery" icon="i-heroicons-magnifying-glass" placeholder="Search sections..." size="lg" />
    </UCard>
    ...
    <!-- Table -->
    <CrudsCrudTable :config="sectionsResource" :items="[...crud.items.value]" :loading="crud.loading.value"
    ...
    <UModal v-model:open="deleteModal" title="Confirm Delete"
      description="This action cannot be undone and will permanently remove the section from the system."
