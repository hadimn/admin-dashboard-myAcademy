...
const itemToDelete = ref<Lessons | null>(null)
...
// Handle delete
const handleDelete = (lesson: Lessons) => {
...
const confirmDelete = async () => {
  ...
  try {
    await crud.deleteItem(itemToDelete.value.lesson_id)
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Manage your Lessons and their permissions
        </p>
      </div>
      <UButton icon="i-heroicons-plus" size="lg" class="w-full sm:w-auto" @click="router.push('/lessons/create')">
        Create Lessons
      </UButton>
    </div>

    <!-- Search -->
    <UCard v-if="lessonsResource.searchable">
      <UInput v-model="searchQuery" icon="i-heroicons-magnifying-glass" placeholder="Search lessons..." size="lg" />
    </UCard>
    ...
    <!-- Table -->
    <CrudsCrudTable :config="lessonsResource" :items="[...crud.items.value]" :loading="crud.loading.value"
    ...
    <UModal v-model:open="deleteModal" title="Confirm Delete"
      description="This action cannot be undone and will permanently remove the lesson from the system."
