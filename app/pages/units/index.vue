<!-- app/pages/units/index.vue -->
<script setup lang="ts">
import { unitsResource, type Units } from '~/config/resources/units'
...
const itemToDelete = ref<Units | null>(null)
...
// Handle delete
const handleDelete = (unit: Units) => {
  itemToDelete.value = unit
...
const confirmDelete = async () => {
  ...
  try {
    await crud.deleteItem(itemToDelete.value.unit_id)
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Manage your Units and their permissions
        </p>
      </div>
      <UButton icon="i-heroicons-plus" size="lg" class="w-full sm:w-auto" @click="router.push('/units/create')">
        Create Unit
      </UButton>
    </div>

    <!-- Search -->
    <UCard v-if="unitsResource.searchable">
      <UInput v-model="searchQuery" icon="i-heroicons-magnifying-glass" placeholder="Search units..." size="lg" />
    </UCard>
    ...
    <!-- Table -->
    <CrudsCrudTable :config="unitsResource" :items="[...crud.items.value]" :loading="crud.loading.value"
    ...
    <UModal v-model:open="deleteModal" title="Confirm Delete"
      description="This action cannot be undone and will permanently remove the unit from the system."
